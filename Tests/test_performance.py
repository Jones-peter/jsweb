...
def test_jsweb_routing_performance():
    ...
    for i in range(50):
        router.add_route(f"/static/page/{i}", lambda req: "OK", methods=["GET"], 
                        endpoint=f"static_{i}")
    
    # Add 50 dynamic routes
    for i in range(50):
        router.add_route(f"/dynamic/<int:id>/resource/{i}", lambda req: "OK", 
                        endpoint=f"dynamic_{i}")
    ...
    assert static_time < 500, f"Static routing too slow: {static_time}ms for 10k requests"
    
    # Dynamic route resolution should be < 1000ms for 10k requests (~100μs per request)
    assert dynamic_time < 1000, f"Dynamic routing too slow: {dynamic_time}ms for 10k requests"
...
def test_jsweb_routing_accuracy_with_dynamic_routes():
    ...
    router.add_route("/users/<int:user_id>/posts/<int:post_id>", handler, 
                    endpoint="user_post")
    ...
    for path, expected_params in test_cases:
        ...
        assert params == expected_params, f"Parameters mismatch for {path}: got {params}, expected {expected_params}"
...
def test_routing_comparison_jsweb_vs_alternatives():
    ...
    class SimpleRouter:
        ...
        def add_route(self, path, handler):
            ...
            regex_path = "^" + re.sub(r'<int:(\w+)>', lambda m: f'(?P<{m.group(1)}>\\d+)', path) + "$"
